(() => {
  ...
  const syncFile = async file => {
    const client = await cloud.ready; const user = await cloud.user();
  ...
  const deleteFile = async id => {
    const client = await cloud.ready; const user = await cloud.user(); if (!client || !user) return false;
  ...
  const loadCloudFiles = async () => {
    const client = await cloud.ready; const user = await cloud.user(); if (!client || !user) return;
  ...
  const uploadFiles = async input => {
    const client = await cloud.ready; const user = await cloud.user(); if (!client || !user || !input.files?.length) return;
    ...
    input.value = ''; if (button) button.classList.remove('uploading'); renderCloudFiles();
  };

  const attach = () => {
    const input = document.querySelector('#localFileInput');
    const uploadLabel = document.querySelector('.file-upload');
    if (uploadLabel) { uploadLabel.childNodes[0].textContent = '+ Upload file '; }
    if (input && input.dataset.cloudFiles !== 'true') { input.dataset.cloudFiles = 'true'; input.addEventListener('change', () => uploadFiles(input)); }
    renderCloudFiles();
  };
  ...
  document.addEventListener('click', async event => {
    ...
    if (row) {
      ...
      event.preventDefault(); const client = await cloud.ready; const { data, error } = await client.storage.from(BUCKET).createSignedUrl(file.storagePath, 60 * 60);
      if (error || !data?.signedUrl) { alert(`Could not open ${file.name}: ${error?.message || 'No download link was created.'}`); return; }
      state.openFile(id); window.open(data.signedUrl, '_blank', 'noopener'); return;
    }
  ...
  cloud.ready.then(client => { if (!client) return; client.auth.onAuthStateChange((event, session) => { if (session?.user) loadCloudFiles(); }); setTimeout(loadCloudFiles, 750); });
